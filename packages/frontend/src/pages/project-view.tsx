...
import type { Project, PullRequest } from '../api.js';
...
export function ProjectView() {
  ...
  useWebSocket((message) => {
    if (
      ...
      void api.prs.list(projectId).then(setPrs);
    }
  });

  useEffect(() => {
    if (!projectId) return;
    void Promise.all([api.projects.get(projectId), api.prs.list(projectId)])
      .then(([proj, prList]) => {
        setProject(proj);
        setPrs(prList);
      })
  ...
  const handleClosePr = async (event: React.MouseEvent, prId: string) => {
  ...
  const handleReopenPr = async (event: React.MouseEvent, prId: string) => {
    event.preventDefault();
    try {
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {pr.status === 'open' && (
                    <button
                      onClick={(event) => {
                        void handleClosePr(event, pr.id);
                      }}
                  ...
                  {pr.status === 'closed' && (
                    <button
                      onClick={(event) => {
                        void handleReopenPr(event, pr.id);
                      }}
                      className="text-xs px-2 py-1 rounded border hover:opacity-80"
                      style={{
                        borderColor: 'var(--color-border)',
