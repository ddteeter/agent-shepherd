...
let pvWsCallback: ((message: WsMessage) => void) | undefined;
...
describe('ProjectView', () => {
  ...
  it('shows loading state initially', () => {
    mockApi.projects.get.mockReturnValue(
      new Promise(() => {
        /* pending */
      }),
    );
    mockApi.prs.list.mockReturnValue(
      new Promise(() => {
        /* pending */
      }),
    );
  ...
  it('calls api.prs.close on close button click', async () => {
    const user = userEvent.setup();
    mockApi.prs.close.mockResolvedValue({
      id: 'pr-1',
      projectId: 'proj-1',
      title: 'Add feature',
      sourceBranch: 'feat/x',
  ...
  it('calls api.prs.reopen on reopen button click', async () => {
    const user = userEvent.setup();
    mockApi.prs.reopen.mockResolvedValue({
      id: 'pr-3',
      projectId: 'proj-1',
      title: 'Closed PR',
      sourceBranch: 'fix/z',
