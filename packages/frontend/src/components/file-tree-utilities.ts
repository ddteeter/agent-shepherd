...
function trieToTreeNodes(node: TrieNode): TreeNode[] {
  ...
  const sortedDirectories = sortedCopy(directories, (a, b) =>
    a.name.localeCompare(b.name),
  );
  const sortedFiles = sortedCopy(fileNodes, (a, b) =>
    a.name.localeCompare(b.name),
  );
...
export function buildGroupedFileTree(
  ...
  const ungrouped = sortedCopy(ungroupedFiles, (a, b) => a.localeCompare(b));
